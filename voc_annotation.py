...
# #   默认指向根目录下的VOC数据集
# #-------------------------------------------------------#
# VOCdevkit_path      = 'VOCdevkit_foodseg103'
...
#             # png__ = Image.open(png_file_name)
#             # # Convert to palette mode and save. Method 3 is "libimagequant"
#             # png__.quantize(colors=256, method=2).save(png_file_name)
#             # png = png__
#             # print("修復完成")
            
#         classes_nums += np.bincount(np.reshape(img_arrays, [-1]), minlength=256)#統計每個數字出現的次數
